...
RUN apt-get update \
    ...
    && git clone -b master --single-branch https://github.com/oleksandrblack/safecoin-seeder.git /root/seeder \
    && cd /root/seeder \
    && make \
    && install -m 755 dnsseed /usr/local/bin/dnsseed-avx \
    && cd \
    && rm -rf /root/seeder \
    && git clone -b portable --single-branch https://github.com/oleksandrblack/safecoin-seeder.git /root/seeder \
    && cd /root/seeder \
    && make \
